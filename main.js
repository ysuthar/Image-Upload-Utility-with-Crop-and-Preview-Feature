...
class Cropper {
     ...
     *  <li>limit {integer}: default to 1024 </li>
     *  <li>canvas {HTMLElement} (required): the cropping canvas element. Instantiation fails if not provided.</li>
     *  <li>realPreview {HTMLElement} (required): the realpreview canvas element. Instantiation fails if not provided.</li>
    ...
    constructor(options) {
        ...
        // Create the cropping square with the handle's size ( handle's size is the bottom-right grabber that is used to resize the cropper )
        this.crop = {
            size: {
        ...
        // Set the preview canvas size, Dividing By 2 so that It is Scaled
    ...
    setImageSource(source) {
        ...
        this.image.onload = e => {
            ...
            this.imageCanvas.ontouchstart = this.touchStart.bind(this);
        };
    }
    ...
    /** @private */
    touchStart(e) {
        // Get the touch position and hold onto it for the expected touchmove
    ...
    /** @private */
    touchDrag(e) {
        if (this.lastEvent.moving || this.lastEvent.resizing) {
            e.preventDefault();
        }
        var rect = e.target.getBoundingClientRect();
        const position = {
        ...
        // Calculate the distance that the user-touch has travelled
        const dx = position.x - this.lastEvent.position.x;
        const dy = position.y - this.lastEvent.position.y;
        // Determine whether we are resizing, moving, or nothing
        if (this.lastEvent.resizing) {
    ...
    resize(dx, dy) {
        ...
        let aspectRatio = (this.crop.size.x / this.crop.size.y);
        let inverseAspectRatio = 1 / aspectRatio;
        // Maintain the aspect ratio
        const amount = dx > dy ? dx : dy;
    ...
    /** @private */
    displayImage() {
        // Resize the original to the maximum allowed size ( Don't required as we have made sure that the image size is fixed at 1024x1024. )
        const ratio =
    ...
    /** @private */
    //  Save the preview to realpreviewed Canvas
    savePreview() {
        const pos = this.crop.pos;
        const size = this.crop.size;
        const imageData = this.c.getImageData(pos.x, pos.y, size.x, size.y);
        ...
        const ctx = this.realPreview.getContext("2d");
        ctx.clearRect(0, 0, this.realPreview.width, this.realPreview.height);
        // Redraw the Image so that Handle's Box is removed
        this.displayImage();
        ctx.drawImage(
            this.imageCanvas,
            // Top left corner coordinates of image
            ...
            0,
            // Width and height of result in canvas
            this.realPreview.width,
            this.realPreview.height
        );
        // Remember the position so that it Handler can be drawn to previous position
    ...
    image.onload = function () {
        try {
            ...
            var editor = new Cropper({
                ...
                realPreview: document.querySelector("#myCanvas1"),
                pos: {
                    x: remember_pos["myCanvas1"] == null ?
                        (this.width - 755) / 2 : remember_pos["myCanvas1"].x,
                    y: remember_pos["myCanvas1"] == null ?
                        (this.height - 450) / 2 : remember_pos["myCanvas1"].y
                }
            });
    ...
    image.onload = function () {
        try {
            ...
            var editor = new Cropper({
                ...
                pos: {
                    x: remember_pos["myCanvas2"] == null ?
                        (this.width - 365) / 2 : remember_pos["myCanvas2"].x,
                    y: remember_pos["myCanvas2"] == null ?
                        (this.height - 450) / 2 : remember_pos["myCanvas2"].y
                }
            });
            // Make sure both were initialised correctly
            if (uploader && editor) {
    ...
    image.onload = function () {
        try {
            ...
            var editor = new Cropper({
                ...
                pos: {
                    x: remember_pos["myCanvas3"] == null ?
                        (this.width - 365) / 2 : remember_pos["myCanvas3"].x,
                    y: remember_pos["myCanvas3"] == null ?
                        (this.height - 212) / 2 : remember_pos["myCanvas3"].y
                }
            });
            // Make sure both were initialised correctly
            if (uploader && editor) {
    ...
    image.onload = function () {
        try {
            ...
            var editor = new Cropper({
                ...
                pos: {
                    x: remember_pos["myCanvas4"] == null ?
                        (this.width - 380) / 2 : remember_pos["myCanvas4"].x,
                    y: remember_pos["myCanvas4"] == null ?
                        (this.height - 380) / 2 : remember_pos["myCanvas4"].y
                }
            });
            // Make sure both were initialised correctly
            if (uploader && editor) {
...
const uploadOneImage = (id, realPreview, name) => {
    // convert Image object to File object
    srcToFile(realPreview.toDataURL(), name, "image/jpeg")
        .then(function (file) {
            var fd = new FormData();
            fd.append("image", file);
    // send the formatted & encrypted file object to Image cloud using API        
    ...
    //  Update the progressBar       
            uploadProgress =
                uploadProgress != null ?
                    uploadProgress +
                    parseInt($(".progress").css("width")) * 0.25 :
                    parseInt($(".progress").css("width")) * 0.25;
            $(".progress-bar").css("width", uploadProgress);
            $(".progress-bar").text(
                uploadProgress / parseInt($(".progress").css("width")) * 100 +
                "%"
            );      
    //  Save the result of that image for later display       
            resultsData[id]=success.image;
    //  if all images are uploaded then showResult      
...
$("#upload").on("click", function () {
    // get and set the names for files to be uploaded
    let name = document.getElementsByClassName("js-fileinput")[0].files[0].name.split(".")[0];
    let name1 = name + "_" + 755 + "x" + 450 + "_" + ".jpg";
    let name2 = name + "_" + 365 + "x" + 450 + "_" + ".jpg";
    let name3 = name + "_" + 365 + "x" + 212 + "_" + ".jpg";
    let name4 = name + "_" + 380 + "x" + 380 + "_" + ".jpg";
...
$(".hover-div").on("click", function () {
    let editThese = this.nextElementSibling.getAttribute("id");
...
const showResult = () => {
    console.log("hello");
    let url = "https://kane99.pythonanywhere.com";
